...
# Create test_logs directory if it does not exist
mkdir -p test_logs

# Figure out where we left off
logs=$(find test_logs -maxdepth 1 -name 'test-*.log' -type f | wc -l)
success=$(grep -E '^PASS$' test_logs/test-*.log | wc -l)
((failed = logs - success))
...
finish() {
    if ! wait "$1"; then
        if command -v terminal-notifier >/dev/null 2>&1 && ((failed == 0)); then
            terminal-notifier -title "Tests started failing" -message "$(pwd)\n$(grep FAIL: -- test_logs/*.log | sed -e 's/.*FAIL: / - /' -e 's/ (.*)//' | sort -u)"
        fi
        ((failed += 1))
    else
        ((success += 1))
...
cleanup() {
    for pid in "${waits[@]}"; do
        kill "$pid"
        wait "$pid"
        rm -rf "test_logs/test-${is[0]}.err" "test_logs/test-${is[0]}.log"
        is=("${is[@]:1}")
    done
    exit 0
}
...
for i in $(seq "$((success+failed+1))" "$runs"); do
    ...
    if [[ -z "$test" ]]; then
        ./tester -test.v 2> "test_logs/test-${i}.err" > "test_logs/test-${i}.log" &
        pid=$!
    else
        ./tester -test.run "$test" -test.v 2> "test_logs/test-${i}.err" > "test_logs/test-${i}.log" &
        pid=$!
    fi
